...
def local_train_net(epsilon_array, nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model = None, prev_model_pool = None, round=None, device="cpu", logger=None):
    ...
    for net_id, net in nets.items():
        ...
        if epsilon_array[net_id] <= 0:  # 如果 epsilon_array[net_id] <= 0，跳过当前 net
            epsilon_array[net_id] = 0
            continue

        if epsilon_array[net_id] < t:
            p = (np.exp(epsilon_array[net_id]) - 1) / (np.exp(t) - 1)
        else:
            p = 1
    ...
    for net_id, net in nets_this_round.items():
        ...
        #为模型添加噪声    
        net = add_noise_to_model(net, epsilon_array[net_id])
        epsilon_array[net_id] = epsilon_array[net_id] - t / 10
    ...
    return epsilon_array, acc_list

def local_train_net_scaffold(epsilon_array, nets, global_model, c_nets, c_global, args, net_dataidx_map, train_dl=None, test_dl = None, device="cpu", logger=None):
    ...
    print("Privacy budget after this round: ", epsilon_array)
    return epsilon_array, nets_list