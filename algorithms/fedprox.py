...
def fedprox_alg(args, epsilon, n_comm_rounds, nets, global_model, party_list_rounds, net_dataidx_map, train_local_dls, test_dl, traindata_cls_counts, moment_v, device, global_dist, logger):
    best_test_acc=0
    ...
    for round in range(n_comm_rounds):
        ...
        # Local update
        epsilon, acc_list = local_train_net(epsilon, nets_this_round, args, net_dataidx_map, train_dl=train_local_dls,test_dl=test_dl, global_model = global_model, device=device, logger=logger)
        global_model.to('cpu')
